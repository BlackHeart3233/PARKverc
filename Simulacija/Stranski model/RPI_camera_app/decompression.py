...
class BralnikBitov:
    def __init__(self, podatki_bytes: bytes):
        self.data = podatki_bytes
        self.i = 0           # byte index
        self.buf = 0         # bit buffer
        self.nbits = 0       # number of bits in buf

    def _fill(self, n: int) -> bool:
        while self.nbits < n:
            if self.i >= len(self.data):
                return False
            self.buf = (self.buf << 8) | self.data[self.i]
            self.i += 1
            self.nbits += 8
        return True

    def preberi_bite(self, n: int):
        # keep compatibility: return '0/1' string for your decoder logic
        if n <= 0:
            return ''
        if not self._fill(n):
            return ''
    ...
    def preberi_int(self, n: int, predznaceno=False):
        if n <= 0:
            return 0
        if not self._fill(n):
            return 0
    ...
    for ch in range(3):
        for i in range(0, v_dopolnjena, 8):
            for j in range(0, s_dopolnjena, 8):
                ...
                blok_idct = idct(idct(blok_dct.T, norm='ortho').T, norm='ortho')
