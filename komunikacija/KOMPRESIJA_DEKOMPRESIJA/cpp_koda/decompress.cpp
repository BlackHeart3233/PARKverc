...
/*
 Razred BitReader omogoča branje poljubnega števila bitov iz
 bajtnega vektorja. Uporablja se pri dekompresiji za branje
 odiranih podatkov bit za bitom v enakem vrstnem
 redu, kot so bili zapisani pri kompresiji.
*/
...
/*
 Funkcija dekodira glavo kompresiranih podatkov.
 Iz bitnega toka prebere osnovne parametre slike in zaporedja:
 višino slike, prvi in zadnji element kumulativnega zaporedja
 ter skupno število elementov
*/
...
/*
 Funkcija inicializira kumulativno zaporedje C.
 Ustvari vektor želene velikosti ter nastavi prvi in zadnji
 element, ki sta bila shranjena v glavi podatkov.
 Ostali elementi se bodo rekonstruirali kasneje.
*/
...
/*
 Funkcija DelC izvede dekodiranje kumulativnega
 zaporedja C. Na osnovi robnih vrednosti in bitnega toka
 postopoma rekonstruira vse manjkajoče elemente zaporedja.
*/
...
/*
 Funkcija inv_JPG_LES izvede inverzno napovedno rekonstrukcijo
 slike (JPEG-LS). Iz napak napovedi E rekonstruira
 originalne slikovne vrednosti z uporabo sosednjih pikslov.
 Rezultat so slikovne vrednosti v int64 obliki.
*/
...
/*
 Funkcija shrani sivinsko sliko v BMP datoteko.
 Ustvari ustrezno BMP glavo, barvno paleto in
 pravilno zapiše slikovne podatke z zahtevanim
 poravnavanjem vrstic na 4 bajte.
*/
...
void saveBMP(const std::string& filename,
    ...
    uint8_t bmpfileheader[14] = {
        ...
        (uint8_t)(filesize >> 24),
        0,0,0,0,
        (uint8_t)((54 + 1024) & 0xFF),
        (uint8_t)(((54 + 1024) >> 8) & 0xFF),
        (uint8_t)(((54 + 1024) >> 16) & 0xFF),
        (uint8_t)(((54 + 1024) >> 24) & 0xFF)
    };
...
/*
 Iz bitnega toka dekodira glavo, rekonstruira kumulativno
 zaporedje, iz njega izračuna napake napovedi, nato pa
 z inverznim napovednim algoritmom obnovi originalno sliko.
 Na koncu vrednosti omeji na interval [0, 255].
*/
...
/*
 Vmesnik za uporabo dekompresije iz Pythona
 Sprejme surove bajte kompresiranih podatkov, ustvari
 BitReader in pokliče glavno dekompresijsko funkcijo.
 Vrne rekonstruirano sliko v obliki uint8_t vektorja.
*/
